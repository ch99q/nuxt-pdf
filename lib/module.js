  ...
  async function build(buildArgs) {
    ...
    try {
      if (buildArgs.generated) {
        ...
        // require.resolve throws when a package is missing instead of returning
        // a falsy value, so nuxt-edge has to be detected with a try/catch.
        let nuxtPackage = 'nuxt'
        try {
          require.resolve('nuxt-edge')
          nuxtPackage = 'nuxt-edge'
        } catch (e) {}
        const {
          loadNuxt
        } = require(nuxtPackage);
      ...
      console.error('nuxt-pdf: ' + e.message)
    }
    ...
    for (let i = 0; i < routes.length; i++) {
      ...
      try {
        ...
        // path.dirname resolves the parent directory using the platform's own
        // separator, so this also works on Windows (backslashes).
