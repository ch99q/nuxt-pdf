...
module.exports = async function PDF(moduleOptions) {
  ...
  const routeMap = require(path.resolve(this.options.buildDir, 'routes.json'))
  ...
  const getUrl = (path, locale) => {
    const chunk = routeMap.find(route => route.path == path)
    if (chunk === undefined) throw new Error('Unable to find route at ' + path)
    ...
    if (i18n.enabled && typeof locale !== 'undefined') {
      ...
      if (i18n.options.differentDomains) {
        if (locale in i18n.domains) {
          ...
          uri = url.replace(/\/$/, '') + route.path
        }
      } else {
        uri = url.replace(/\/$/, '') + route.path
      }
    } else {
      uri = url.replace(/\/$/, '') + routes[0].path
    }
  ...
  async function build() {
    ...
    for (let i = 0; i < options.routes.length; i++) {
      ...
      try {
        ...
        else document.setTitle(await page.title())
